...
org\_name: Red de ejemplo

site\_title: Looking Glass

site\_description: Consultas de enrutamiento
...
&#x20;     display\_name: Red de ejemplo

&#x20;   credential:

&#x20;     username: lookingglass

&#x20;     password: "REEMPLAZAR\_LOCALMENTE"
...
&#x20;       display\_name: Global

&#x20;       default: true

&#x20;       ipv4:

&#x20;         source\_address: 192.0.2.10
...
La dirección `address` se usa para SSH. Las direcciones `source\_address` se usan como origen de ping y traceroute y deben existir en el router; pueden ser distintas de la dirección SSH.
...
El valor `REEMPLAZAR\_LOCALMENTE` es un marcador literal, no una variable de entorno. Escribe el secreto únicamente en el servidor y restringe la lectura del archivo al usuario del servicio.
...
Cambia `web.text.title\_mode` a `logo\_subtitle` para mostrar el logo y el subtítulo. Puedes usar archivos distintos para ambos temas. Los archivos deben existir antes de arrancar.
...
La validación compara el prefijo BGP y el ASN de origen del `AS\_PATH`. Arista tiene salida estructurada automática: omite `structured\_output` en el dispositivo. En el código revisado de 1.0.4, establecerlo explícitamente en `true` puede desactivarlo por un defecto de su validador.
...
Environment="NODE\_OPTIONS=--max-old-space-size=4096"
...
| Node sin memoria | Configurar `NODE\_OPTIONS=--max-old-space-size=4096`. |
...
| `TypeError` en `get\_node\_version` | Node no está en el `PATH` del servicio. |
...
| RPKI no verificado | Comprobar salida estructurada, AS\_PATH, acceso HTTPS y respuesta de la API externa. |
